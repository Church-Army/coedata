...
```{r tests-coe_parish_in_context}
test_that("coe_parish_in_context works", {
  expect_true(inherits(coe_parish_in_context, "function"))
  
  test <- coe_parish_in_context("370047", nomis_codes = "TS001")
...
A convenient wrapper around `coe_stats` for getting parish-level statistics for parishes of your choosing

```{r function-coe_parish_stats}
...
#' 
#'@export 
coe_parish_stats <- function(nomis_code, parish_codes, ...){
  
  if(!rlang::is_missing(parish_codes)) out <- coe_stats(nomis_code, level = "parish", areas = parish_codes, ...)
  else out <- coe_stats(nomis_code, level = "parish", ...)
  out
}
```
...
A convenient wrapper around `coe_stats` for getting diocese-level statistics for dioceses of your choosing

```{r function-coe_diocese_stats}
#' Obtain dicoese level statistics on a particular topic for any diocese of your choosing
#' 
#' @param nomis_code The nomis code for the required topic
...
A convenient wrapper around `coe_stats` for getting national-level stats for a topic of your choosing

```{r function-coe_england_stats}
